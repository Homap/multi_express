...
if (file.exists("diffExpr.P1e-2_C2.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("diffExpr.P1e-2_C2.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("diffExpr.P1e-2_C2.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
    primary_data = as.matrix(primary_data)
}
source("/sw/bioinfo/trinity/2.13.2/rackham/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/sw/bioinfo/trinity/2.13.2/rackham/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/sw/bioinfo/trinity/2.13.2/rackham/Analysis/DifferentialExpression/R/pairs3.R")
source("/sw/bioinfo/trinity/2.13.2/rackham/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
...
write.table(data, file="diffExpr.P1e-2_C2.matrix.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="diffExpr.P1e-2_C2.matrix.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("diffExpr.P1e-2_C2.matrix.log2.centered.sample_cor_matrix.pdf")
...
", "diffExpr.P1e-2_C2.matrix.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
...
heatmap_data = data
pdf("diffExpr.P1e-2_C2.matrix.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "diffExpr.P1e-2_C2.matrix.log2.centered" ) , ColSideColors=sampleAnnotations)
dev.off()
save(list=ls(all=TRUE), file="diffExpr.P1e-2_C2.matrix.RData")
